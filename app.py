...
def score_job(title, desc):
    ...
    s = 40
    if any(t in title.lower() for t in ["head","director","vp","chief","cmo"]): s += 15
    elif any(t in title.lower() for t in ["senior","manager"]): s += 8
...
def get_tags(title):
    ...
    elif "pr" in tl or "comm" in tl: tags.append("Comms")
    elif "growth" in tl or "performance" in tl: tags.append("Growth")
    return tags

QUERIES = [
    "Head of Marketing UAE", "Marketing Director Dubai", "CX Director UAE",
    "Brand Director Dubai", "Senior Marketing Manager UAE",
    "Head of Marketing Saudi Arabia", "Marketing Director Qatar"
]

def fetch_rapidapi():
    if not RAPIDAPI_KEY: return []
    jobs, headers = [], {"X-RapidAPI-Key": RAPIDAPI_KEY, "X-RapidAPI-Host": "jsearch.p.rapidapi.com"}
    for q in QUERIES:
        try:
            r = requests.get("https://jsearch.p.rapidapi.com/search", headers=headers,
                params={"query": q, "page": "1", "num_pages": "1", "date_posted": "month"}, timeout=8)
            if r.status_code == 200:
                for j in r.json().get("data", []):
                    ...
                    jobs.append({
                        "title": title, "company": j.get("employer_name", "") or "",
                        "location": f"{city}, {country}".strip(", "), "description": desc,
                        "applyUrl": j.get("job_apply_link", "") or "", "source": "LinkedIn",
                        "score": score_job(title, desc),
                        "posted": (j.get("job_posted_at_datetime_utc", "") or "")[:10],
                        "tags": get_tags(title), "type": "RapidAPI"
                    })
            time.sleep(0.5)
        except Exception as e: print(f"RapidAPI failed: {q} - {e}")
    seen, out = set(), []
    for j in jobs:
...
def get_cache():
    try:
        ...
        if datetime.now() - datetime.fromisoformat(d["at"]) < timedelta(hours=6): return d["jobs"]
    except: pass
    return None
...
def get_linkedin_cache():
    try:
        with open(LINKEDIN_FILE) as f: return json.load(f)
    except: return {"jobs": [], "pushed_at": None, "count": 0}
...
@app.route("/")
def root(): return jsonify({"status": "ok", "version": "4.0"})

@app.route("/health")
def health():
    li = get_linkedin_cache()
    return jsonify({
        "status": "ok", "rapidapi_configured": bool(RAPIDAPI_KEY),
...
def push_jobs():
    ...
    if secret != PUSH_SECRET: return jsonify({"error": "Unauthorized"}), 401
    try:
        raw_jobs = (request.json or {}).get("jobs", [])
        if not raw_jobs: return jsonify({"error": "No jobs provided"}), 400
        filtered, seen = [], set()
        for j in raw_jobs:
            ...
            filtered.append({
                ...
                "source": "LinkedIn", "score": score_job(title, desc),
                "posted": j.get("time_posted") or j.get("posted") or "",
                "tags": get_tags(title), "type": "LinkedIn via Apify"
            })
        filtered = sorted(filtered, key=lambda x: x["score"], reverse=True)
        set_linkedin_cache(filtered)
        return jsonify({"ok": True, "received": len(raw_jobs), "filtered": len(filtered), "pushed_at": datetime.now().isoformat()})
    except Exception as e:
